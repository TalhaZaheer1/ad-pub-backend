...
const registerCompany = async (data) => {
  const { companyName, companySlug, firstName, lastName, email, password, timezone } = data;
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const user = await tx.user.create({
      data: {
        ...
        email,
        passwordHash,
        role: Role.COMPANY_ADMIN, // The first user of a registered company is a COMPANY_ADMIN
        isActive: true,
      },
    });
    ...
    await Promise.all(
      defaultSizes.map(d => tx.adSize.create({
